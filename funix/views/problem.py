...
class ProblemBeta(ProblemMixin, TitleMixin, SingleObjectFormView):
    ...
    def form_valid(self, form, **kwargs):
        ...
        self.new_submission.save()
        template = loader.get_template('funix/submission/submission.html')
        context = {}
        submission = self.new_submission
        context['submission'] = self.new_submission
        problem = submission.problem
        context['problem']  = problem
        context['testcases_map'] = map_test_cases(problem.cases.all())
        context['old_submission'] = self.new_submission
        ...
        context['form'] = self.get_form(form_class=None)
        context['default_lang'] = self.default_language
        # return HttpResponse(template.render(context, self.request))
        return JsonResponse({"html": template.render(context, self.request), "submission_id_secret": submission.id_secret, "submission_id": submission.id, "last_msg": event.last()})
        return super().form_valid(form)
...
# -------------------- problem details ----------------------------
class ProblemDetailsBeta(DetailView):
    template_name = 'funix/problem/problem-details.html'
    model = Problem

    def get_object(self, *args, **kwargs):
        if 'code' not in self.request.GET:
            return HttpResponseBadRequest()
        self.object = get_object_or_404(Problem, code=self.request.GET.get('code'))
        return self.object
